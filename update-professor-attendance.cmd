...
echo ==^> Updating professor attendance records...
docker run --rm ^
  -e "PGPASSWORD=%DB_PASSWORD%" ^
  -v "%SCRIPT_DIR%\db:/db:ro" ^
  postgres:15-alpine ^
  psql -h "%DB_HOST%" -U "%DB_USER%" -d "%DB_NAME%" -v ON_ERROR_STOP=1 -f /db/actualiza_asistencia_profesores.sql
if errorlevel 1 exit /b 1

echo ==^> Professor attendance updated.
